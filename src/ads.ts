...
type SplitString<T extends string, A extends string[] = []> =
...
class Helper {
    ...
    static processResponse(res: GoogleAppsScript.URL_Fetch.HTTPResponse) {
        const isSuccess = res.getResponseCode() === 200;
        isSuccess || makeLogger().log(res.getContentText());
        return isSuccess;
    }
}
...
class AdsHelper extends Helper {
    ...
    static search<R>(
        ...
        const { results = [], nextPageToken }: SearchGoogleAdsResponse<R> =
            success ? JSON.parse(resText) : { results: [] };
    ...
    `;
        ...
        const walkAccount = (id: string, managerId = id): Customer[] => {
            ...
            const { results } = this.bulkSearch<{ customerClient: Customer }>(
                query,
                {
            ...
            return results.flatMap(({ customerClient }) => {
                ...
                return [
                    customerClient,
                    ...walkAccount(customerClient.id, managerId),
                ];
            });
        };
    ...
    static handleCallUploadError<T extends string>(
        ...
        return errors.every(
            ({ errorCode: { conversionUploadError }, message }) => {
    ...
    static mutateCustomerClientLink({
        ...
        const { content, success } = AdsHelper.fetchAPI(
            `${this.getCustomerResourceName(
                managerId
...
class AdsMutatateOperation extends Operation {
    ...
    setUpdate<T>(resource: T) {
        ...
        this.operation.update_mask =
            AdsMutatateOperation.resourceToProtoBuf(resource);
    }
...
<script>
  ...
  gtag('config', 'AW-${conversionTrackingId}');
    ...
    return installTag([], tagName, workspacePath, {
        type: "html",
        parameter: [
            {
                key: "html",
